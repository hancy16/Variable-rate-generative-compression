...
class Utils(object):
    ...
    @staticmethod
    def run_diagnostics(model,G_loss, D_loss, Dis_loss,Mat_loss,config, directories, sess, saver, train_handle, start_time, epoch, name, G_loss_best, D_loss_best):
        t0 = time.time()
        improved = ''
    ...
    @staticmethod
    def single_plot(epoch,reconstruction, global_step, sess, model, handle, name, config, single_compress=False):
    ...
    @staticmethod
    def single_plot_compress(epoch,thres,image,merge_op,res,reconstruction, global_step, sess, model, handle, name, config ,height,width,single_compress=False):

        #real = model.example
        gen = reconstruction[0]

        ori_image = tf.image.resize_images(gen, tf.stack([height,width],axis=0))
        # Generate images from noise, using the generator network.
       
        ori_image,w_hat,w_hat_1,w_hat_2,w_mask_1,w_mask_2 = sess.run([ori_image]+res[0:-3], feed_dict={model.training_phase:False,model.thres:thres,model._x:image,model.handle: handle})
        #print("Start PCA!")
        #w_pca,pca_latent = test_pca.Redim(w_hat,10)
        #print("End PCA!")
        #w_pca = np.expand_dims(w_pca,axis=0)
            #print(w_pca.shape)
        ##g_pca = sess.run(res[-1], feed_dict={model.training_phase:False,model._pca:w_pca})
        #pca_latent  = pca_latent  - np.min(pca_latent)
        #pca_latent  = pca_latent.astype(int)
        #print("".format(np.min(pca_latent),np.max(pca_latent).astype(int)))
        #print("Start Encoding!")
        #ori_nums = AAC.ori_compress(pca_latent,1+np.max(pca_latent).astype(int))
        #print("End Encoding!")        
        #g = g_pca
        #g[:,0:H:2,0:W:2,:] = final_pca[0]
        #g[:,0:H:2,1:W:2,:] = final_pca[1]
        #g[:,1:H:2,0:W:2,:] = final_pca[2]
        #g[:,1:H:2,1:W:2,:] = final_pca[3]
        #r = r[0:1]
        w_hat_1 = w_hat_1.astype(int)
        w_mask_1 = w_mask_1.astype(int)
        w_hat_1 = w_hat_1 -1

        w_hat_2 = w_hat_2.astype(int)
        w_mask_2 = w_mask_2.astype(int)
        w_hat_2 = w_hat_2 -1

        #w_hat_3 = w_hat_3.astype(int)
        #w_mask_3 = w_mask_3.astype(int)
        #w_hat_3 = w_hat_3 -1

        #w_hat_4 = w_hat_4.astype(int)
        #w_mask_4 = w_mask_4.astype(int)
        #w_hat_4 = w_hat_4 -1

        #test_pca.Redim(w_hat)
        #print(w_hat)
        #w_hat =w_hat.astype(float)
        #w_hat  = (w_hat-1.5)/1.5
        ...
        #for im, imtype in zip([np.squeeze(image),g], ['real', 'gen']):
         #   im = ((im+1.0))/2  # [-1,1] -> [0,1]
          #  im = np.squeeze(im)
           # im = im[:,:,:3]
            #images.append(im)
        ...
        #plt.axis('off')
           ...
           # f.savefig(name, format='png', dpi=720, bbox_inches='tight', pad_inches=0)
        #else:
         #   f.savefig("{}/gan_compression_{}_epoch{}_step{}_thres{}_{}_comparison.png".format(directories.samples, name, epoch,
          #      global_step, thres,imtype), format='png', dpi=720, bbox_inches='tight', pad_inches=0)
        #plt.gcf().clear()
        #plt.close(f)
        
        ori_image = np.squeeze(ori_image)
        ori_image = ((ori_image+1.0))/2
        
        matplotlib.image.imsave("{}/Thres{}_Image{}.png".format(directories.samples,thres,global_step),ori_image)
	
        nums=0
        #nums = AAC.compress([w_hat_1,w_hat_2],[w_mask_1,w_mask_2],5) 
        if thres>0:
            nums = nums+ AAC.ori_compress([w_mask_1,w_mask_2],2) 
        #misc.toimage(g,cmin=0,cmax=1.0).save("Image{}.png".format(global_step))
        #print("Original BPP:{}".format(ori_nums/cols/rows))
        return nums/height/width,w_hat #AAC.compress(w_hat,w_mask_1,3)/cols/rows


    @staticmethod
    def Dense_upsample_1(x):
    ...
    def Dense_upsample(img):
        ...
        W = shape_HWC[2]
        C = int(img.shape[3])
        assert C%4==0, 'Channels shoud be interger times 4'
        img = tf.transpose(img,[0,2,1,3])
        img = tf.reshape(img,[N,W,tf.cast(H*2,dtype=tf.int32),tf.cast(C/2,dtype=tf.int32)])
        img = tf.transpose(img,[0,2,1,3])
        img = tf.reshape(img,[N,tf.cast(H*2,dtype=tf.int32),tf.cast(W*2,dtype=tf.int32),tf.cast(C/4,dtype=tf.int32)])
        
        return img

    @staticmethod
    def Dense_downsample(img):
        assert len(img.get_shape())==4,'Data Dimension should be 4'
        shape_HWC = tf.shape(img)
        N = int(img.shape[0])
        H = shape_HWC[1]
        W = shape_HWC[2]
        C = int(img.shape[3])
        img = tf.reshape(img,[N,H,tf.cast(W/2,dtype=tf.int32),tf.cast(2*C,dtype=tf.int32)])
        img = tf.transpose(img,[0,2,1,3])
        img = tf.reshape(img,[N,tf.cast(W/2,dtype=tf.int32),tf.cast(H/2,dtype=tf.int32),tf.cast(4*C,dtype=tf.int32)])
        img = tf.transpose(img,[0,2,1,3])
        return img
