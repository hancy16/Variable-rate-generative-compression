...
class Network(object):
    ...
    @staticmethod
    def encoder(x, config, training, C, reuse=tf.AUTO_REUSE, actv=tf.nn.relu, scope='image'):
        """
        ...
        with tf.variable_scope('encoder_{}'.format(scope), reuse=reuse):
            ...
            res = residual_block(out, f[2], actv=actv)
            res = residual_block(res, f[2], actv=actv)
            res = residual_block(res, f[2], actv=actv)
            res = residual_block(res, f[2], actv=actv)
            res = residual_block(res, f[2], actv=actv)


            out = conv_block(res, filters=f[3], kernel_size=3, strides=2, actv=actv)
    ...
    @staticmethod
    def quantizer(w, config, reuse=tf.AUTO_REUSE, temperature=1, L=5, scope='image'):
        """
        ...
        with tf.variable_scope('quantizer_{}'.format(scope, reuse=reuse)):
            ...
            w_hard = tf.cast(tf.argmin(tf.abs(w_stack - centers), axis=-1), tf.float32) + 1.0
            smx = tf.nn.softmax(-1.0/temperature * tf.abs(w_stack - centers), dim=-1) 
            # Contract last dimension
            w_soft = tf.einsum('ijklm,m->ijkl', smx, centers)  + 3.0 # w_soft = tf.tensordot(smx, centers, axes=((-1),(0)))
    ...
    @staticmethod
    def decoder(w_bar, config, training, C, reuse=tf.AUTO_REUSE, actv=tf.nn.relu, channel_upsample=960):
        """
    ...
    @staticmethod
    def discriminator(x, config, training, reuse=tf.AUTO_REUSE, actv=tf.nn.leaky_relu, use_sigmoid=False, ksize=4):
    ...
    def multiscale_discriminator(x, config, training, actv=tf.nn.leaky_relu, use_sigmoid=False, 
        ksize=4, mode='real', reuse=tf.AUTO_REUSE):
        # x is either generator output G(z) or drawn from the real data distribution
    ...
    @staticmethod
    def dcgan_generator(z, config, training, C, reuse=tf.AUTO_REUSE, actv=tf.nn.relu, kernel_size=5, upsample_dim=256):
        """
        ...
        with tf.variable_scope('noise_generator', reuse=reuse):
            ...
            with tf.variable_scope('fc1', reuse=reuse):
                h2 = tf.layers.dense(z, units=2 * 4 * upsample_dim, activation=actv, kernel_initializer=init)  # cifar-10
                h2 = tf.layers.batch_normalization(h2, **kwargs)
                h2 = tf.reshape(h2, shape=[-1, 2, 4, upsample_dim])
    ...
    @staticmethod
    def dcgan_discriminator(x, config, training, reuse=tf.AUTO_REUSE, actv=tf.nn.relu):
        # x is either generator output G(z) or drawn from the real data distribution
        init =  tf.contrib.layers.xavier_initializer()
        kwargs = {'center':True, 'scale':True, 'training':training, 'fused':True, 'renorm':False}
        print('Shape of x:', x.get_shape().as_list())
    ...
    @staticmethod
    def critic_grande(x, config, training, reuse=tf.AUTO_REUSE, actv=tf.nn.relu, kernel_size=5, gradient_penalty=True):
        # x is either generator output G(z) or drawn from the real data distribution
        init =  tf.contrib.layers.xavier_initializer()
        kwargs = {'center':True, 'scale':True, 'training':training, 'fused':True, 'renorm':False}
        print('Shape of x:', x.get_shape().as_list())
    ...
    @staticmethod
    def wrn(x, config, training, reuse=tf.AUTO_REUSE, actv=tf.nn.relu):
    ...
    @staticmethod
    def old_encoder(x, config, training, C, reuse=tf.AUTO_REUSE, actv=tf.nn.relu):
        """
        Process image x ([512,1024]) into a feature map of size W/16 x H/16 x C
         + C:       Bottleneck depth, controls bpp
         + Output:  Projection onto C channels, C = {2,4,8,16}
